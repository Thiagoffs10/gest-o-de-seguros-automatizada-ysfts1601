migrate(
  (app) => {
    console.log('[MIGRATION 0054] Iniciando sincronização de legado e competências...')
    ...
    try {
      ...
      console.log('[MIGRATION 0054] Erro ao buscar comissao_recebimentos sem competência:', err)
      emptyCompRecs = []
    }

    console.log(
      `[MIGRATION 0054] Total de recebimentos com competência vazia encontrados: ${emptyCompRecs.length}`,
    )
    ...
    for (let i = 0; i < emptyCompRecs.length; i++) {
      ...
      if (!dataRec) {
        console.log(`[MIGRATION 0054] Recebimento ${recId} sem data_recebimento, pulando.`)
        continue
      }
      ...
      if (parts.length >= 2) {
        ...
        console.log(
          `[MIGRATION 0054] Recebimento ${recId} (apólice ${rec.getString('policy')}): competência ANTES='${compAntiga}', DEPOIS='${compInferida}' (data_recebimento: ${dataRec})`,
        )
        ...
        try {
          ...
          console.log(`[MIGRATION 0054] Erro ao salvar recebimento ${recId}:`, err)
        }
      }
    }

    console.log(
      `[MIGRATION 0054] Parte 1 concluída: ${recsAtualizadosCount} recebimentos tiveram competência preenchida.`,
    )
    ...
    // Criterioso: só onde não há ambiguidade.
    // =========================================================================
    let prevsPendentes = []
    try {
      ...
      console.log('[MIGRATION 0054] Erro ao buscar comissoes_previstas pendentes:', err)
      prevsPendentes = []
    }

    console.log(
      `[MIGRATION 0054] Total de previsões com status 'Pendente' encontradas: ${prevsPendentes.length}`,
    )

    let prevsAtualizadasCount = 0

    // Agrupar previsões pendentes por apólice para análise consistente
    const prevsByPolicy = {}
    ...
    for (let k = 0; k < policyIds.length; k++) {
      ...
      try {
        ...
        console.log(`[MIGRATION 0054] Erro ao buscar recebimentos da apólice ${polId}:`, err)
        continue
      }

      if (polRecs.length === 0) continue

      // Buscar apólice para conferir status e dados
      let polRecord = null
      try {
        polRecord = app.findFirstRecordByData('policies', 'id', polId)
      } catch (_) {}
      ...
      // Se há apenas 1 previsão para a apólice e o total recebido cobre o valor previsto
      if (allPrevsForPolicy.length === 1) {
        ...
        if (singlePrev.getString('status') === 'Pendente') {
          ...
          if (vPrev > 0 && totalBrutoRecebido >= vPrev - 0.01) {
            console.log(
              `[MIGRATION 0054] Previsão única ${singlePrev.id} (apólice ${polId}): status ANTES='Pendente', DEPOIS='Recebida' (previsto=${vPrev}, recebido=${totalBrutoRecebido})`,
            )
            ...
            try {
              ...
              console.log(
                `[MIGRATION 0054] Erro ao atualizar status da previsão ${singlePrev.id}:`,
                err,
              )
            }
          }
        ...
        // Múltiplas previsões: verificar recebimento por vínculo direto ou por competência exata sem ambiguidade
        for (let pIdx = 0; pIdx < polPrevs.length; pIdx++) {
          ...
          if (brutoDireto >= vPrev - 0.01) {
            console.log(
              `[MIGRATION 0054] Previsão ${prevId} (vínculo direto, apólice ${polId}): status ANTES='Pendente', DEPOIS='Recebida' (previsto=${vPrev}, recebido_direto=${brutoDireto})`,
            )
            ...
            try {
              ...
              console.log(`[MIGRATION 0054] Erro ao salvar status da previsão ${prevId}:`, err)
            }
            continue
          }
          ...
          if (compPrev) {
            ...
            // Verificar se não há outra previsão da apólice com a MESMA competência (para não haver ambiguidade)
            ...
            if (prevsMesmaComp === 1 && brutoComp >= vPrev - 0.01) {
              console.log(
                `[MIGRATION 0054] Previsão ${prevId} (competência unívoca ${compPrev}, apólice ${polId}): status ANTES='Pendente', DEPOIS='Recebida' (previsto=${vPrev}, recebido_comp=${brutoComp})`,
              )
              prev.set('status', 'Recebida')
              try {
                app.save(prev)
    ...
    console.log(
      `[MIGRATION 0054] Parte 2 concluída: ${prevsAtualizadasCount} previsões tiveram status atualizado para 'Recebida'.`,
    )
    console.log('[MIGRATION 0054] Migração 0054 finalizada com sucesso.')
  },
  (app) => {
    // Reversão de dados operacionais pontuais não é necessária nem recomendada
    console.log('[MIGRATION 0054] Downnoop')
  },
)
