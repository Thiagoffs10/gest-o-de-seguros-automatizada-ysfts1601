...
import { AlertCircle, CheckCircle2, ShieldAlert, ArrowRight, RefreshCw } from 'lucide-react'
import {
  SistemaAlerta,
  getSistemaAlertas,
...
export const AlertasCentral: React.FC<AlertasCentralProps> = ({ modulo, onVerApolice }) => {
  ...
  const carregarAlertas = async () => {
    ...
    try {
      ...
      const data = await getSistemaAlertas(filter)
      setAlertas(data)
    } finally {
      setLoading(false)
    }
  ...
  if (alertas.length === 0 && !loading) {
    return null
  }

  return (
    <Card className="border-amber-200 dark:border-amber-900/50 bg-amber-50/40 dark:bg-amber-950/20 mb-6">
